  ...
  private startHeroAnimationSequence(): void {
    ...
    this.autoNavTimeline.to(animObj, {
      ...
      onUpdate: () => {
        ...
        gsap.set(pathEl, { opacity: 1, strokeDasharray: liveLen, strokeDashoffset: liveLen - currentLen });
      ...
      const tween = gsap.to(dummy, {
        ...
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            if (progress > 0.01) {
              const liveLen = pathEl.getTotalLength() || length;
              const currentLen = liveLen * progress;
              gsap.set(pathEl, { opacity: 1, strokeDasharray: liveLen, strokeDashoffset: liveLen - currentLen });
